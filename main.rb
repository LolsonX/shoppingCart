...
def list_basket
  blank_screen
  @win.refresh
  @co.cart.each_with_index {|product, index| @win << "#{index+1} #{product.to_s} \n"}
  @win.refresh
...
def add_to_basket
  ...
  blank_screen
  @win.refresh
  @products.each_with_index {|product, index| @win << "#{index+1} #{product.to_s} \n"}
...
def list_promotions
  ...
  @win.refresh
  @win << "Total Discounts\n"
  @promotions.total_discounts.each_with_index {|p, i| @win << "\t#{i+1} #{p.to_s}\n"}
  @win << "Multiple Items Discounts\n"
  @promotions.product_count_discounts.each_with_index {|p, i| @win << "\t#{i+1} #{p.to_s}\n"}
  @win << "Press any key to continue\n"
  @win.getch
end
